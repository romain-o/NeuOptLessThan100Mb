...
class CVRPDataset(Dataset):
    def __init__(self, filename=None, size=20, num_samples=10000, offset=0, distribution=None, DUMMY_RATE = None, CVRPLib_paths=None, coords_norm_factor = None):
        ...
        if filename is not None:
            ...
            assert coords_norm_factor is not None, 'Missing coords_norm_factor'
            for instance_path in CVRPLib_paths:
                instance = vrplib.read_instance(instance_path)
                
                capacity = instance['capacity']
                norm_coords = torch.from_numpy(instance['node_coord']).float() / coords_norm_factor
                ...
                n_padding = int(np.ceil(instance_size * DUMMY_RATE)) - 1
